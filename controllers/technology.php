     ...
     foreach ($Fetch_FChat_Result->result_array() as $row)
     {
         ...
         if($row['kms_forum'] == 8)
        {
          ...
          /**$data['Menu_item_00_01_09'] = "I.T"; */
           ...
           {
            $data['IT']  = 0; $data['SC'] = 0; $data['PD'] = 0; $data['CM'] = 0;$data['FC'] = 0;$data['AD'] = 0;$data['GN']  = 0;$data['DS']  = 0;
           }
        
     }
     //Then we call the chat forum name function as below
    
     $this->load->view('kms_tech_forum',$data);
    }else{
      ...
      if($this->session->userdata('kms_access_status') == 1){
         ...
         if(isset($kms_single_forum_chart_msg) && $kms_single_forum_chart_msg != "")
         {
          ...
          $SaveQueryResult = $this->Chat->send_fchat_message($kms_single_forum_chart_rec_forum,"KMS",$kms_single_forum_chart_msg,$Input_fchat_time,$Input_fchat_date,$Input_fchat_sender_id);
            //add flash data 
           redirect('Technology/Forum/'.$kms_single_forum_chart_rec_forum.'');        
        ...
        if($SaveQueryResult != 0)
         {
          ...
          //add flash data 
          redirect('Technology/Forum/'.$Input_fchat_Forum);
  ...
  $data['Menu_item_00_01_09'] = "I.T";
      ...
      //The submenu item for i.t document side
       ...
       $data['Menu_item_04_03'] = "Files Hub ";
        ...
        $this->load->view('kms_tech_files',$data);
      }else{
...
if($this->session->userdata('kms_access_status') == 1){
    ...
    //The view data to be used
    $data['Home_tab_title'] = "KMS | I.T | Files";
    $data['Side_menu_title'] = "KMS- I.T";
    //The menu items on the I.T home side
    $data['Menu_item_00'] = "Communicate";
    $data['Menu_item_04'] = "Documents";
  ...
  public function Tech_file_directory_dep($Dep_id)
  {
    if($this->session->userdata('kms_access_status') == 1){

    //The view data to be used
    $data['Home_tab_title'] = "KMS | I.T | File";
    $data['Side_menu_title'] = "KMS- I.T";
    //The menu items on the I.T home side
    $data['Menu_item_00'] = "Communicate";
    $data['Menu_item_04'] = "Documents";
    ...
    $this->load->helper('url');
    $this->load->view('techfile_directory',$data);
  ...
  //The function that will handle the news section on the administration dept
  public function Dept_Directory()
  {
    if($this->session->userdata('kms_access_status') == 1){
    ...
    foreach ($Fetch_File_count_Result->result_array() as $row){}
        ...
        $this->session->set_userdata('side_menu_dept2','Technology');
        //echo "New from the administration section";
        $this->load->view('file_directory_tech',$data);
      }else{
        $this->load->helper('url');
        redirect('/');
          }
